...
class DSIDispersion():
    ...
    Args:
        ...
        tanhsinh_integrator (optional): Instance of tanhsinh integrator class. If None, use quadpack for integrals
    """
    def __init__(self,
                 dust_to_gas_ratio,
                 stokes_range,
        ...
        self.z0 = z0_over_H*sound_speed_over_eta
    ...
    def matrix_inverse_A(self, w):
        ...
        d = lambda x: self.kx*self.ux(x) + self.kz*self.uz(x) - w - 1j/x
    ...
    def matrix_AD(self, A, w):
        ...
        d31 = lambda x: (1j/x)*duz(x)*self.kx*denom
        d32 = lambda x: 0*x
        d33 = lambda x: (1j/x)*(duz(x)*self.kz*denom - 1)
    ...
    def matrix_VAD(self, AD, w):
        ...
        denom = lambda x:1/(w - self.kx*self.ux(x) - self.kz*self.uz(x) + \
        ...
        v31 = lambda x: duz(x)*self.kx*denom(x)
        v32 = lambda x: 0*x
        v33 = lambda x: 1 + duz(x)*self.kz*denom(x)
    ...
    def matrix_W(self, w):
        ...
        w31 = lambda x: duz(x)*self.kx*denom(x)
        w32 = lambda x: 0*x
        w33 = lambda x: duz(x)*self.kz*denom(x) - 1
    ...
    def matrix_M(self, w):
        ...
        return np.asarray([[m11, m12, m13], [m21, m22, m23], [m31, m32, m33]])
